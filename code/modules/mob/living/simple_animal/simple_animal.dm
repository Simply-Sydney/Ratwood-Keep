...
/mob/living/simple_animal/hostile/user_unbuckle_mob(mob/living/M, mob/user)
	...
	if(!move_after(M,time2mount, target = src) || islamia(user))
		fall_T(src, user)
	..()
	update_icon()
...
/mob/living/proc/fall_T(var/atom/M, mob/living/user)
	user.Paralyze(50)
	user.Stun(50)
	playsound(user.loc, 'sound/foley/zfall.ogg', 100, FALSE)
	user.visible_message(span_danger("[user] falls off [M]!"))
